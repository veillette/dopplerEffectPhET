...
export class TrailManager {
  ...
  public updateTrails(
    ...
    this.sourceTrail.visible = this.visibleProperty.value && sourceTrailPoints.length > 0;
    this.observerTrail.visible = this.visibleProperty.value && observerTrailPoints.length > 0;
  ...
  private updateTrail(
    ...
    const oldestPoint = this.modelViewTransform.modelToViewPosition(points[0].position);
    trailShape.moveToPoint(oldestPoint);

    // Add each subsequent point
    for (let i = 1; i < points.length; i++) {
      const point = this.modelViewTransform.modelToViewPosition(points[i].position);
    ...
    const startPoint = this.modelViewTransform.modelToViewPosition(points[0].position);
    const endPoint = this.modelViewTransform.modelToViewPosition(points[points.length - 1].position);
    ...
    gradient.addColorStop(0.5, new Color(
      color.r,
      color.g,
      color.b,
      0.4
    ));
    gradient.addColorStop(1, new Color(
      color.r,
      color.g,
      color.b,
      0.8
