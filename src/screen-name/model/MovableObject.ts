...
export class MovableObject {
  ...
  public updatePosition(dt: number): void {
    if (this.movingProperty.value) {
      ...
      if (velocity.magnitude < PHYSICS.MIN_VELOCITY_MAG) {  // PHYSICS.MIN_VELOCITY_MAG in m/s
        this.movingProperty.value = false;
      }
    } else if (!this.movingProperty.value && this.velocityProperty.value.magnitude > 0) {
      // Apply velocity decay when not actively moving
      const velocity = this.velocityProperty.value;  // in meters per second (m/s)
      this.velocityProperty.value = velocity.timesScalar(PHYSICS.VELOCITY_DECAY);  // in meters per second (m/s)
    }
  }
